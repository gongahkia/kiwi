...
return {
  ...
  kitty_image_invalidations_are_a_distinct_bounded_reason = function()
    local invalidation = Invalidation.new()
    invalidation:request("kitty_images")
    Assert.equal(table.concat(invalidation:snapshot().reasons, ","), "kitty_images")
    invalidation:consume_success(1)
    Assert.equal(invalidation:due(1), false)
  end,
