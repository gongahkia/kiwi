...
return {
  ...
  vt_terminal_render_updates_apply_reverse_screen_video_without_mutating_semantic_cells = function()
    local terminal = Terminal.new({ columns = 2, rows = 1 })
    terminal:write("A")
    local original = terminal:begin_render_update()
    local original_cell = original:cell(0, 0)
    terminal:end_render_update(false)
    terminal:write("\27[?5h")
    local view = terminal:begin_render_update()
    local cell = view:cell(0, 0)
    local blank = view:cell(1, 0)
    Assert.equal(cell.fg, original_cell.bg)
    Assert.equal(cell.bg, original_cell.fg)
    Assert.equal(blank.fg, original_cell.bg)
    Assert.equal(blank.bg, original_cell.fg)
    terminal:end_render_update(false)
  end,
