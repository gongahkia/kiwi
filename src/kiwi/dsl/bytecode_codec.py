...
def _encode_constant(writer: _Writer, constant: IntegerValue | BooleanValue | UnitValue) -> None:
...
def _decode_module(reader: _Reader) -> BytecodeModule:
    ...
    constants = ConstantPool(
        tuple(_decode_constant(reader) for _ in range(reader.items("constant count")))
    )
    table = FunctionTable(
        tuple(
            FunctionTableEntry(
    ...
    functions = tuple(_decode_function(reader) for _ in range(reader.items("function count")))
...
def _decode_constant(reader: _Reader) -> IntegerValue | BooleanValue | UnitValue:
    tag = reader.u8()
...
def _decode_function(reader: _Reader) -> BytecodeFunction:
...
def _decode_type(reader: _Reader, depth: int) -> DslType:
    ...
    if tag == _TYPE_FUNCTION:
        parameters = tuple(
            _decode_type(reader, depth + 1)
            for _ in range(reader.items("function-type parameter count"))
        )
        return FunctionType(parameters, _decode_type(reader, depth + 1))
    raise _DecodeError(
        BytecodeDecodeCode.INVALID_VALUE,
        tag_offset,
