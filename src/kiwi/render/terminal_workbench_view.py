"""Bitmap rendering for the non-authoritative Terminal briefing and workbench."""
...
def _validate_color(color: tuple[int, int, int]) -> None:
    if not isinstance(color, tuple) or len(color) != 3:
        raise ValueError("Terminal workbench palette color must be an RGB tuple")
    if any(not isinstance(component, int) or isinstance(component, bool) for component in color):
        raise ValueError("Terminal workbench palette color components must be integers")
    if any(component < 0 or component > 255 for component in color):
        raise ValueError("Terminal workbench palette color components must be 0 through 255")


@dataclass(frozen=True, slots=True)
class TerminalWorkbenchPalette:
    """Fixed restrained tactical-terminal colours for the Terminal flow."""
...
DEFAULT_TERMINAL_WORKBENCH_PALETTE = TerminalWorkbenchPalette()


@dataclass(frozen=True, slots=True)
class TerminalWorkbenchRenderResult:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.phase, TerminalFlowPhase):
            raise TypeError("Terminal workbench render phase is invalid")
        if not isinstance(self.line_count, int) or isinstance(self.line_count, bool):
            raise TypeError("Terminal workbench render line count must be an integer")
        if self.line_count <= 0:
            raise ValueError("Terminal workbench render line count must be positive")


def render_terminal_workbench(
    surface: pygame.Surface,
    font: BitmapFont,
    workbench: TerminalWorkbench,
    *,
    scale: int = 1,
    palette: TerminalWorkbenchPalette = DEFAULT_TERMINAL_WORKBENCH_PALETTE,
    source_palette: SourcePalette = DEFAULT_SOURCE_PALETTE,
    compact_sidebar: bool = False,
    completions: tuple[str, ...] = (),
) -> TerminalWorkbenchRenderResult:
    """Render briefing or source-review state without changing editor or authority state."""
    if not isinstance(surface, pygame.Surface):
        raise TypeError("Terminal workbench rendering requires a pygame surface")
    if not isinstance(font, BitmapFont):
        raise TypeError("Terminal workbench rendering requires a bitmap font")
    if not isinstance(workbench, TerminalWorkbench):
        raise TypeError("Terminal workbench rendering requires workbench state")
    if not isinstance(scale, int) or isinstance(scale, bool) or scale < 1:
        raise ValueError("Terminal workbench render scale must be positive")
    if not isinstance(palette, TerminalWorkbenchPalette):
        raise TypeError("Terminal workbench render palette is invalid")
    if not isinstance(source_palette, SourcePalette):
        raise TypeError("Terminal workbench source palette is invalid")
    if not isinstance(compact_sidebar, bool):
        raise TypeError("Terminal workbench compact sidebar flag must be boolean")
    if not isinstance(completions, tuple) or any(
        not isinstance(completion, str) or not completion for completion in completions
    ):
        raise TypeError("Terminal workbench completions must be non-empty text")
    surface.fill(palette.background)
    if workbench.phase is TerminalFlowPhase.BRIEFING:
...
def _render_briefing(
    ...
    workbench: TerminalWorkbench,
    scale: int,
    palette: TerminalWorkbenchPalette,
) -> TerminalWorkbenchRenderResult:
    ...
    return TerminalWorkbenchRenderResult(workbench.phase, len(lines))
...
def _render_workbench(
    ...
    return TerminalWorkbenchRenderResult(workbench.phase, line_count)
...
def _source_from_line(source: SourceFile, first_line: int) -> SourceFile:
    ...
    if not isinstance(first_line, int) or isinstance(first_line, bool) or first_line < 1:
        raise ValueError("Terminal source view line must be positive")
    lines = source.text.splitlines(keepends=True)
    if first_line > len(lines):
        raise ValueError("Terminal source view line exceeds source length")
