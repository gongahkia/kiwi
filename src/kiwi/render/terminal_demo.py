"""pygame-ce shell for the runnable Terminal causal-drill usability flow."""
...
class TerminalDemoPalette:
    ...
    workbench: TerminalWorkbenchPalette
    source: SourcePalette


_CYAN_PALETTE = TerminalDemoPalette(
    _BACKGROUND,
    _PANEL,
    _BORDER,
    _HEADING,
...
_AMBER_PALETTE = TerminalDemoPalette(
    (20, 14, 8),
    (35, 26, 16),
    (178, 129, 61),
    (255, 204, 105),
...
_PHOSPHOR_PALETTE = TerminalDemoPalette(
    (8, 17, 11),
    (13, 31, 20),
    (70, 136, 85),
    (142, 255, 162),
...
_MAROON_PALETTE = TerminalDemoPalette(
    (24, 7, 11),
    (43, 13, 20),
    (143, 53, 69),
    (255, 183, 187),
...
_WHITE_PALETTE = TerminalDemoPalette(
    (242, 239, 232),
    (255, 255, 252),
    (91, 103, 117),
    (23, 37, 54),
...
_BLACK_PALETTE = TerminalDemoPalette(
    (5, 5, 6),
    (17, 17, 19),
    (93, 96, 102),
    (244, 244, 240),
...
_PALETTES = {
    TerminalColorScheme.CYAN: _CYAN_PALETTE,
    TerminalColorScheme.AMBER: _AMBER_PALETTE,
    TerminalColorScheme.PHOSPHOR: _PHOSPHOR_PALETTE,
    TerminalColorScheme.MAROON: _MAROON_PALETTE,
    TerminalColorScheme.WHITE: _WHITE_PALETTE,
    TerminalColorScheme.BLACK: _BLACK_PALETTE,
}
...
def run_terminal_demo() -> int:
    ...
    try:
        ...
        while running:
            ...
            elif controller.screen is not TerminalDemoScreen.LIVE_PREVIEW:
                next_preview_step_at = now + _PREVIEW_STEP_MILLISECONDS
...
def _handle_event(
    controller: TerminalDemoController, event: pygame.event.Event
...
def _deploy_pressed(controller: TerminalDemoController, event: pygame.event.Event) -> bool:
    ...
    *,
    ...
    if controller.screen is TerminalDemoScreen.GUIDE:
        render_terminal_tutorial(
            ...
            font,
            controller.tutorial,
            palette=TerminalTutorialPalette(
                palette.background,
                palette.heading,
...
def _render_mission(
    ...
    if controller.current_run is None:
        raise AssertionError("Terminal mission screen has no run")
...
def _render_debrief(
    ...
    if controller.current_run is None:
        raise AssertionError("Terminal debrief screen has no run")
    surface.fill(palette.background)
    debrief = controller.current_run.debrief
    if isinstance(debrief, TerminalDebrief):
        render_terminal_debrief(
            surface,
            font,
            debrief,
            (8, 8),
            palette=TerminalDebriefPalette(
                palette.heading,
                palette.normal,
                palette.notice,
            ),
...
def _render_comparison(
    ...
    if controller.comparison is None:
        raise AssertionError("Terminal comparison screen has no comparison")
    surface.fill(palette.background)
...
def _render_results(
    ...
    if not controller.result_history.results:
        raise AssertionError("Terminal result screen requires a completed local attempt")
...
def _render_panel(
    ...
    font: BitmapFont,
    lines: tuple[str, ...],
    palette: TerminalDemoPalette = _CYAN_PALETTE,
) -> None:
    line_height = font.measure("M")[1]
    height = len(lines) * line_height + 8
    pygame.draw.rect(surface, palette.panel, (4, 4, surface.get_width() - 8, height))
...
def _deploy_label(controller: TerminalDemoController) -> str:
    """Return the selected deployment shortcut label for a non-editing screen."""
    return "Cmd+R" if controller.input_mode is TerminalInputMode.STANDARD else "F5"
...
def _input_mode_label(input_mode: TerminalInputMode) -> str:
    """Return one compact selected input-mode label."""
    return "standard keys" if input_mode is TerminalInputMode.STANDARD else "function keys"
...
def _handle_click(
    controller: TerminalDemoController, position: tuple[int, int], font: BitmapFont
) -> TerminalDemoController:
    if controller.screen is TerminalDemoScreen.INPUT_SETUP:
        ...
        if standard.collidepoint(position):
            return controller.choose_input_mode(TerminalInputMode.STANDARD)
        if function_keys.collidepoint(position):
            return controller.choose_input_mode(TerminalInputMode.FUNCTION_KEYS)
...
def _move_editor_to_pointer(
    ...
    if updated.screen is not TerminalDemoScreen.LIVE_PREVIEW:
        return updated
...
def _fit_text(font: BitmapFont, text: str, width: int) -> str:
    ...
    if width <= 0:
        raise ValueError("Terminal text width must be positive")
...
def _palette_for(color_scheme: TerminalColorScheme) -> TerminalDemoPalette:
    """Return the explicit presentation palette for one controller scheme."""
    if not isinstance(color_scheme, TerminalColorScheme):
        raise TypeError("Terminal color scheme is invalid")
    return _PALETTES[color_scheme]


def _tactical_palette(palette: TerminalDemoPalette) -> TacticalPalette:
    """Project the active terminal theme across the renderer-only tactical view."""
    if not isinstance(palette, TerminalDemoPalette):
        raise TypeError("Terminal tactical palette requires a demo palette")
