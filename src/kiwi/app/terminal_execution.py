"""Deterministic Terminal deployment, tick execution, and signal queueing."""
...
class TerminalSignal(StrEnum):
    """The limited strategic signal vocabulary offered during Terminal."""
...
class TerminalMissionExecution:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.state, MissionState):
            raise TypeError("Terminal execution requires mission state")
        if not isinstance(self.initial_state, MissionState):
            raise TypeError("Terminal execution requires initial mission state")
        if self.initial_state.tick > self.state.tick:
            raise ValueError("Terminal initial mission state must not follow current state")
        if not isinstance(self.clock, FixedTickClock):
            raise TypeError("Terminal execution requires a fixed tick clock")
        if not isinstance(self.policy_bindings, PolicyBindings):
            raise TypeError("Terminal execution requires policy bindings")
        ...
        if not isinstance(self.player_entity_ids, tuple) or len(self.player_entity_ids) != 4:
            raise ValueError("Terminal execution requires four player entity IDs")
        if any(not isinstance(entity_id, EntityId) for entity_id in self.player_entity_ids):
            raise TypeError("Terminal player entity IDs are invalid")
        if self.player_entity_ids != tuple(
            sorted(self.player_entity_ids, key=lambda item: item.value)
        ):
            raise ValueError("Terminal player entity IDs must be ordered")
        ...
        if any(entity_id not in state_entity_ids for entity_id in self.player_entity_ids):
            raise ValueError("Terminal player entity IDs must belong to mission state")
        if (
            not isinstance(self.lockdown_tick, int)
            or isinstance(self.lockdown_tick, bool)
            or self.lockdown_tick < 0
        ):
            raise ValueError("Terminal lockdown tick is invalid")
        if (
    ...
    def request_start(self) -> TerminalMissionExecution:
        """Queue the player's start request for the next authoritative tick."""
        if self.state.phase is not MissionPhase.PREPARED:
            raise ValueError("Terminal mission is not prepared")
        if self.queued_commands:
            raise ValueError("Terminal mission start is already queued")
        return self._queue(StartMission(self._next_header()))

    def queue_signal(
        self, signal: TerminalSignal, target: EntityId | None = None
        ...
        if self.state.phase is not MissionPhase.ACTIVE:
            raise ValueError("Terminal mission must be active before issuing a signal")
        if not isinstance(signal, TerminalSignal):
            raise TypeError("Terminal signal is invalid")
        if target is not None and not isinstance(target, EntityId):
            raise TypeError("Terminal signal target must be an entity ID or absent")
        if target is not None and target not in self.player_entity_ids:
            raise ValueError("Terminal signals may target only deployed player operatives")
        return self._queue(IssueSignal(self._next_header(), SignalName(signal.value), target))

    def advance(self) -> TerminalMissionExecution:
    ...
    def _next_header(self) -> CommandHeader:
        if self.next_command_sequence > MAX_STABLE_ID:
            raise ValueError("Terminal command sequence is exhausted")
        return CommandHeader(self.state.tick, self.next_command_sequence, CommandSource.PLAYER)

    def _queue(self, command: ExternalCommand) -> TerminalMissionExecution:
        return replace(
            self,
...
type TerminalMissionExecutionResult = TerminalMissionExecution | TerminalPolicyFailure


def build_terminal_mission_execution(
    mission: MissionData,
    workbench: TerminalWorkbench,
    hostile_sources: tuple[SourceFile, ...],
) -> TerminalMissionExecutionResult:
    """Compile the current workbench bundle and prepare one headless Terminal run."""
    if not isinstance(mission, MissionData):
        raise TypeError("Terminal execution requires mission data")
    if mission.mission_id != "terminal":
        raise ValueError("Terminal execution requires the Terminal mission")
    if not isinstance(workbench, TerminalWorkbench):
        raise TypeError("Terminal execution requires a workbench")
    if workbench.phase is not TerminalFlowPhase.WORKBENCH:
        raise ValueError("Terminal workbench must be open before deployment")
    players = build_terminal_player_setup(mission, workbench.sources)
    if isinstance(players, TerminalPolicyFailure):
        return players
    hostiles = build_terminal_hostile_setup(players, hostile_sources)
    if isinstance(hostiles, TerminalPolicyFailure):
        return hostiles
    return _execution_from_setups(mission, players, hostiles, workbench.sources, hostile_sources)


def build_terminal_mission_presentation(
    ...
    if not isinstance(execution, TerminalMissionExecution):
        raise TypeError("Terminal mission presentation requires execution state")
    ...
    if queued_signals:
        ...
        signal_status = TerminalSignalStatus(
    ...
    if len(objectives) != 1:
        raise AssertionError("Terminal mission presentation requires one objective")
    return TerminalMissionPresentation(
        ...
        signal_status,
        build_terminal_sound_cues(execution.last_tick_events),
    )


def _execution_from_setups(
    mission: MissionData,
    players: TerminalPlayerSetup,
    hostiles: TerminalHostileSetup,
    ...
    if len(lockdown_events) != 1:
        raise AssertionError("Terminal mission requires exactly one lockdown event")
    return TerminalMissionExecution(
...
def _validate_policy_sources(sources: tuple[SourceFile, ...], bindings: PolicyBindings) -> None:
    if not isinstance(sources, tuple) or any(
        ...
        raise TypeError("Terminal policy sources must be immutable source files")
    ...
    if source_file_ids != tuple(sorted(source_file_ids)) or len(set(source_file_ids)) != len(
        ...
        raise ValueError("Terminal policy sources must be unique and source-file ordered")
    ...
    if source_file_ids != binding_file_ids:
        raise ValueError("Terminal policy sources must match deployed policy bindings")
