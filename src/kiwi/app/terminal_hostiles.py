"""Headless Terminal hostile-roster construction through the same DSL boundary."""
...
class TerminalHostileRole(StrEnum):
...
class TerminalHostileLoadout:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.role, TerminalHostileRole):
            raise ValueError("Terminal hostile loadout requires a role")
        if not isinstance(self.policy_file_id, str) or not self.policy_file_id:
            raise ValueError("Terminal hostile policy file ID must be text")
        if any(
        ...
        if names != tuple(sorted(names)) or len(set(names)) != len(names):
            raise ValueError("Terminal hostile capabilities must be lexically ordered and unique")


@dataclass(frozen=True, slots=True)
class TerminalHostileSetup:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.state, MissionState):
            raise ValueError("Terminal hostile setup requires mission state")
        if not isinstance(self.policy_bindings, PolicyBindings):
            raise ValueError("Terminal hostile setup requires policy bindings")
        if len(self.state.entities) != 7 or len(self.state.weapons.entries) != 7:
            raise ValueError("Terminal hostile setup requires four players and three hostiles")
        if tuple(binding.entity_id for binding in self.policy_bindings.entries) != tuple(
            ...
            raise ValueError("Terminal hostile setup bindings must match every deployed entity")
...
TERMINAL_HOSTILE_LOADOUTS = (
    TerminalHostileLoadout(
        TerminalHostileRole.PATROL,
        "examples/policies/terminal/hostile_patrol.dtr",
        6_500,
        1_000,
        3,
    ...
    TerminalHostileLoadout(
        TerminalHostileRole.SENTRY,
        "examples/policies/terminal/hostile_sentry.dtr",
        4_000,
        0,
        4,
    ...
    TerminalHostileLoadout(
        TerminalHostileRole.WARDEN,
        "examples/policies/terminal/hostile_warden.dtr",
        6_500,
        -1_000,
        5,
...
def build_terminal_hostile_setup(
    players: TerminalPlayerSetup, policy_sources: tuple[SourceFile, ...]
) -> TerminalHostileSetup | TerminalPolicyFailure:
    """Compile and bind three project-authored hostile policies after player deployment."""
    if not isinstance(players, TerminalPlayerSetup):
        raise TypeError("Terminal hostile setup requires the player setup")
    if not isinstance(policy_sources, tuple) or len(policy_sources) != len(
        TERMINAL_HOSTILE_LOADOUTS
    ):
        raise ValueError("Terminal hostile setup requires three immutable policy sources")
    artifacts: list[CompiledArtifact] = []
    for source, loadout in zip(policy_sources, TERMINAL_HOSTILE_LOADOUTS, strict=True):
        if not isinstance(source, SourceFile):
            raise TypeError("Terminal hostile policies must be source files")
        if source.file_id.value != loadout.policy_file_id:
            raise ValueError("Terminal hostile policy source IDs must match the canonical roster")
    ...
    for loadout, artifact in zip(TERMINAL_HOSTILE_LOADOUTS, artifacts, strict=True):
