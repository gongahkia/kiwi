"""Read-only, single-construct Terminal language-guide state."""
...
class TerminalTutorialConstruct(StrEnum):
    """The supported scout-policy constructs introduced by the Terminal guide."""
...
class TerminalTutorialLesson:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.construct, TerminalTutorialConstruct):
            raise TypeError("Terminal tutorial lesson construct is invalid")
        if not isinstance(self.title, str) or not self.title:
            raise ValueError("Terminal tutorial lesson title must be text")
        ...
        if not isinstance(self.try_line, str) or not self.try_line:
            raise ValueError("Terminal tutorial lesson try line must be text")


@dataclass(frozen=True, slots=True)
class TerminalTutorial:
    """An immutable selected lesson from the fixed Terminal language guide."""

    lessons: tuple[TerminalTutorialLesson, ...]
    selected_lesson_index: int = 0

    def __post_init__(self) -> None:
        if not isinstance(self.lessons, tuple) or not self.lessons:
            raise ValueError("Terminal tutorial requires immutable lessons")
        if any(not isinstance(lesson, TerminalTutorialLesson) for lesson in self.lessons):
            raise TypeError("Terminal tutorial lessons are invalid")
        constructs = tuple(lesson.construct for lesson in self.lessons)
        if len(set(constructs)) != len(constructs):
            raise ValueError("Terminal tutorial constructs must be unique")
        if (
            ...
            raise ValueError("Terminal tutorial selection is invalid")

    @property
    def selected_lesson(self) -> TerminalTutorialLesson:
        """Return the current lesson without consulting editor or authority state."""
        return self.lessons[self.selected_lesson_index]

    def select_construct(self, construct: TerminalTutorialConstruct) -> TerminalTutorial:
        """Select one fixed lesson without changing source, compilation, or authority."""
        if not isinstance(construct, TerminalTutorialConstruct):
            raise TypeError("Terminal tutorial construct is invalid")
        ...
        raise ValueError("Terminal tutorial construct is unavailable")

    def next_lesson(self) -> TerminalTutorial:
    ...
    def previous_lesson(self) -> TerminalTutorial:
...
def _validate_lines(lines: tuple[str, ...], label: str) -> None:
    if not isinstance(lines, tuple) or not lines:
        raise ValueError(f"Terminal tutorial lesson {label} must be immutable non-empty text")
    if any(not isinstance(line, str) or not line for line in lines):
        raise ValueError(f"Terminal tutorial lesson {label} must contain text")


TERMINAL_LANGUAGE_TUTORIAL = TerminalTutorial(
    (
        TerminalTutorialLesson(
