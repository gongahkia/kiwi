"""Safe source-revision guidance from a selected Glasshouse injury."""
...
@dataclass(frozen=True, slots=True)
class GlasshouseGuidedRevision:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.workbench, GlasshouseWorkbench):
            raise TypeError("guided revision requires a Glasshouse workbench")
        if self.workbench.phase is not GlasshouseFlowPhase.WORKBENCH:
            raise ValueError("guided revision requires an open Glasshouse workbench")
...
type GuidedRevisionResult = (
    GlasshouseGuidedRevision | GuidedRevisionUnavailable | TraceNavigationUnavailable
)
...
def guided_source_revision(
    ...
    if not isinstance(debrief, GlasshouseDebrief):
        raise TypeError("guided revision requires a Glasshouse debrief")
    ...
    if isinstance(selected, GlasshouseDebriefUnavailable):
    ...
    return GlasshouseGuidedRevision(
        selected.selected_node_id,
        pane,
...
def _editable_entities(
    archive: ReplaySourceArchive, workbench: GlasshouseWorkbench
) -> tuple[EntityId, ...]:
...
def _select_workbench_source(workbench: GlasshouseWorkbench, file_id: str) -> GlasshouseWorkbench:
    for policy in workbench.policies:
        if policy.source.file_id.value == file_id:
            return workbench.select_policy(policy.role)
    raise AssertionError("eligible historical source has no Glasshouse workbench policy")
