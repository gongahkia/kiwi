...
def test_terminal_debrief_renders_selected_injury_and_causal_chain() -> None:
    source = """
import pygame
...
from kiwi.render.terminal_debrief_view import (
    DEFAULT_TERMINAL_DEBRIEF_PALETTE,
...
canvas.fill((0, 0, 0))
result = render_terminal_debrief(canvas, font, debrief, (0, 0))
pixels = {canvas.get_at((x, y))[:3] for x in range(640) for y in range(240)}
assert result.line_count > 3
assert DEFAULT_TERMINAL_DEBRIEF_PALETTE.heading in pixels
assert DEFAULT_TERMINAL_DEBRIEF_PALETTE.selected in pixels
quit_pygame()
"""
