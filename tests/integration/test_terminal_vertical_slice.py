...
SCOUT_PATH = REPOSITORY_ROOT / "examples" / "policies" / "terminal" / "scout.dtr"
ENEMY_PATH = (
...
class TerminalVerticalSliceFixture:
...
def test_terminal_vertical_slice_accepts_the_complete_failure_to_revision_loop() -> None:
    workbench = build_terminal_workbench(_sources(_player_file_ids())).open_workbench()
    ...
    assert build_terminal_mission_presentation(active).snapshot.phase == "active"
    assert (
        TERMINAL_LANGUAGE_TUTORIAL.select_construct(
    ...
    debrief = terminal_debrief(baseline_trace)
    ...
    assert isinstance(guided, TerminalGuidedRevision)
...
def _player_file_ids() -> tuple[str, ...]:
    return tuple(loadout.policy_file_id for loadout in TERMINAL_PLAYER_LOADOUTS)


def _hostile_file_ids() -> tuple[str, ...]:
    return tuple(loadout.policy_file_id for loadout in TERMINAL_HOSTILE_LOADOUTS)
...
def _fixture(scout_artifact: CompiledArtifact) -> TerminalVerticalSliceFixture:
    ...
    return TerminalVerticalSliceFixture(
        state,
        player,
        scout_source,
        enemy_source,
...
def _bindings(
    fixture: TerminalVerticalSliceFixture, scout_artifact: CompiledArtifact
) -> PolicyBindings:
...
def _record(fixture: TerminalVerticalSliceFixture, bindings: PolicyBindings) -> RecordedReplay:
...
def _source_archive(
    ...
    fixture: TerminalVerticalSliceFixture,
