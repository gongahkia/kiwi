...
from kiwi.ui.glasshouse_debrief import (
    ...
    GlasshouseDebriefUnavailableCode,
    glasshouse_debrief,
)


def test_glasshouse_debrief_selects_each_retained_injury_with_its_causal_chain() -> None:
    trace = _trace()
    result = glasshouse_debrief(trace)

    assert isinstance(result, GlasshouseDebrief)
    ...
    assert isinstance(selected, GlasshouseDebrief)
    assert selected.chain.detail.event.node_id == TraceNodeId(3)
    assert selected.chain.ancestors[0].node.node_id == TraceNodeId(1)


def test_glasshouse_debrief_reports_missing_injury_consequence() -> None:
    result = glasshouse_debrief(CausalTrace(b"d" * 32, TraceLevel.SUMMARY, (), ()))

    assert isinstance(result, GlasshouseDebriefUnavailable)
    assert result.code is GlasshouseDebriefUnavailableCode.NO_INJURY_RETAINED


def test_glasshouse_debrief_rejects_an_injury_not_retained_by_its_trace() -> None:
    result = glasshouse_debrief(_trace())
    ...
    assert isinstance(selected, GlasshouseDebriefUnavailable)
    assert selected.code is GlasshouseDebriefUnavailableCode.INJURY_NOT_RETAINED
