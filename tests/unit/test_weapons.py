...
def test_weapon_and_aim_stores_are_canonical_and_use_implicit_zero_aim() -> None:
    ...
    suppressions = SuppressionStore((SuppressionState(EntityId(1), 2_000),))
    ...
    assert aim.with_quality(EntityId(2), 1).quality_for(EntityId(2)) == 1
    assert aim.with_quality(EntityId(1), 0).quality_for(EntityId(1)) == 0
    assert suppressions.suppression_for(EntityId(1)) == 2_000
    assert suppressions.suppression_for(EntityId(2)) == 0
    assert suppressions.with_suppression(EntityId(2), 1).suppression_for(EntityId(2)) == 1
    assert suppressions.with_suppression(EntityId(1), 0).suppression_for(EntityId(1)) == 0
    ...
    with pytest.raises(ValueError, match="entity-ID ordered"):
        SuppressionStore((SuppressionState(EntityId(2), 1), SuppressionState(EntityId(1), 1)))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (lambda: SuppressionState(EntityId(1), 0), "between one"),
        (lambda: SuppressionStore().with_suppression(EntityId(1), 10_001), "zero and 10,000"),
...
def test_weapon_and_aim_authority_state_round_trips_and_hashes() -> None:
    ...
    assert hash_canonical_state(state) != hash_canonical_state(
        replace(
            state, weapons=WeaponStore(), aim_states=AimStore(), suppressions=SuppressionStore()
        )
    )
    assert encoded
...
def test_mission_state_rejects_orphan_or_unallocated_weapon_and_aim_values() -> None:
    ...
    with pytest.raises(ValueError, match="suppression states must belong"):
        MissionState(suppressions=SuppressionStore((SuppressionState(EntityId(1), 1),)))
