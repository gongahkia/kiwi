...
def test_glasshouse_hostiles_use_the_same_compiled_policy_boundary() -> None:
    mission = load_mission_file(MISSION_PATH)

    assert isinstance(mission, MissionData)
    players = build_glasshouse_player_setup(
        mission, _sources(tuple(loadout.policy_file_id for loadout in GLASSHOUSE_PLAYER_LOADOUTS))
    )
    assert isinstance(players, GlasshousePlayerSetup)
    setup = build_glasshouse_hostile_setup(
        players, _sources(tuple(loadout.policy_file_id for loadout in GLASSHOUSE_HOSTILE_LOADOUTS))
    )

    assert isinstance(setup, GlasshouseHostileSetup)
