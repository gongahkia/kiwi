...
def test_policy_invocation_uses_pre_state_memory_and_canonical_entity_order() -> None:
    ...
    bindings = PolicyBindings(
        (
            PolicyBinding(first.entity_id, artifact, FunctionId(0), initial_memory),
            PolicyBinding(second.entity_id, artifact, FunctionId(0), initial_memory),
        )
    )
...
def test_policy_bindings_reject_noncanonical_and_unbound_entries() -> None:
    artifact = _two_argument_policy_artifact()
    binding = PolicyBinding(EntityId(1), artifact, FunctionId(0), _memory("initial"))

    with pytest.raises(ValueError, match="entity-ID ordered"):
        PolicyBindings(
            (PolicyBinding(EntityId(2), artifact, FunctionId(0), _memory("two")), binding)
        )
...
def test_policy_binding_requires_a_two_argument_policy_entry_point() -> None:
    ...
    with pytest.raises(ValueError, match="observation and memory"):
        PolicyBinding(EntityId(1), artifact, FunctionId(0), _memory("initial"))

    with pytest.raises(ValueError, match="policy entry point"):
        PolicyBinding(EntityId(1), _helper_and_policy_artifact(), FunctionId(0), _memory("initial"))
