...
from kiwi.sim.fallback import (
    PolicyFallbackKind,
    commit_policy_decisions,
...
def test_policy_decision_commit_persists_memory_and_deployed_version() -> None:
    state, entity = add_entity(MissionState(), WorldPosition(WorldSubunits(1), WorldSubunits(2)))
    ...
    bindings = PolicyBindings(
        (
            PolicyBinding(
                entity.entity_id,
                _updated_memory_policy_artifact(),
                FunctionId(0),
                MEMORY_SCHEMA,
                initial_memory,
            ),
    ...
    decisions = resolve_policy_decisions(
        validate_policy_evaluations(invoke_policies(state, bindings), bindings)
    )

    committed = commit_policy_decisions(decisions.state, decisions, bindings)

    assert committed.policy_memory.memory_for(entity.entity_id) == _memory("next")
    assert (
        committed.policy_versions.version_for(entity.entity_id)
        == bindings.entries[0].policy_version
    )
    with pytest.raises(ValueError, match="prior authority allocations"):
        commit_policy_decisions(state, decisions, bindings)
...
def _updated_memory_policy_artifact() -> CompiledArtifact:
    source = SourceFile(
        SourceFileId("updated-policy.dtr"),
        "type SelfObservation = { entity_id: Int, position: Position }\n"
        "type Observation = { self: SelfObservation, tick: Int }\n"
        "type Memory = { label: String }\n"
        "type Wait = { duration: Duration }\n"
        "type Decision = { intentions: List<Wait>, memory: Memory }\n"
        "policy decide(observation: Observation, memory: Memory) -> Decision = "
        'Decision { intentions = [], memory = Memory { label = "next" } }\n',
    )
    checked = check(resolve(parse(lex(source)).module))
