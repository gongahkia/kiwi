...
SCOUT_FILE_ID = SourceFileId("examples/policies/glasshouse/scout.dtr")
...
def test_guided_revision_prefers_editable_player_source_and_preserves_compilation() -> None:
    ...
    assert isinstance(result, GlasshouseGuidedRevision)
...
def _workbench(*, scout_text: str = SCOUT_TEXT) -> GlasshouseWorkbench:
    return create_glasshouse_workbench(
        (
            WorkbenchPolicy(
