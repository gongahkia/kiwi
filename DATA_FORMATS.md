...
`KWI-STATE\0` version `4` is the canonical mission-state payload. It uses a
...
exact payload. Decoders reject versions `1`, `2`, and `3`, unsupported versions, malformed
...
`KWI-STATE\0`, 16-bit format version `4`, tick, phase, entities, optional map
geometry, entity-ID ordered policy-memory records,
...
bytes plus its selected entry function ID. Versions `1`, `2`, and `3` are
