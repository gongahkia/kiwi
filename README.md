...
Run `make terminal` (or `uv run --extra dev python -m kiwi.terminal`) for the local pygame practice sequence. It opens with an operator-at-terminal boot sequence, then detects the runtime platform and offers standard (`Cmd+G`, `Cmd+T`, `Cmd+Enter`, `Cmd+R`) or function-key (`F1`, `F2`, `Cmd`/`Ctrl`+`Enter`, `F5`) controls. First Link is a persistent, replayable onboarding drill; completion opens an unrestricted practice dossier for Terminal, Glasshouse, and Redline. Each level has a skippable first-entry-per-session intro, then opens directly into the editable deterministic preview; the editor-only screen is not part of the player flow. The forecast labels known, inferred, and unknown information, while `Cmd`/`Ctrl`+`K` opens a trace-backed source-to-outcome receipt. Redline's `Cmd`/`Ctrl`+`1` Advance signal is a recorded policy input and `Cmd`/`Ctrl`+`2` Hold sends no advance signal. `Compile + run` reopens the preview after a one-frame jacking-in loader. `L` opens the persistent local data-shard codex; `Cmd`/`Ctrl`+`Shift`+`C` toggles CRT and `Cmd`/`Ctrl`+`Shift`+`F` reduces flicker. CRT, codex, sprites, audio, and cutscenes are presentation/application data only and never affect simulation or replay results.
