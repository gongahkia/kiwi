...
Run `make glasshouse` (or `uv run --extra dev python -m kiwi.glasshouse`) for the local pygame Glasshouse causal drill. It demonstrates the manual usability-test loop: press `Enter` to open the workbench; `F1` for the language guide; `F5` to deploy the deterministic two-tick drill; `Enter` for its injury debrief; `R` to follow causal evidence to source; `F2` to select Lark's `1m` caution literal; type `0m`; then press `F5` and `C` to inspect the compatible replay comparison. `Ctrl`/`Cmd`+`Enter` compiles the selected policy, `Tab` changes policy, and `Esc` returns to the workbench or exits from the briefing. The drill is local-only and does not submit telemetry or test responses.
