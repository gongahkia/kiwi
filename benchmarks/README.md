...
independent samples. Each sample times one deterministic steady-state frame and
separately measures one GC-stopped steady-state frame for bytes allocated per frame.
The small allocation window prevents the deliberately allocation-heavy full-grid
kinetic fixture from retaining an unbounded measurement buffer.
...
clean fixture from the same run.
