...
colour and cursor-colour operations; a strict XDG configuration file with themes, font controls,
reload, and an explicit OSC 52 write-only opt-in; supported Kitty keyboard flags
1, 2, 8, and 16; and a bounded tab/split workspace topology. The live GLFW app now
...
tabs continue to service their PTYs. Linux now also has a bounded AT-SPI
Application/Text provider for the active pane, with registry/query and
text-change-event smoke evidence; it has not yet been validated with a screen
reader.
...
accessibility validation and cross-platform adapters, and macOS support. Kiwi
now publishes a deliberately narrow experimental C ABI, but it lacks the broad
render/input/effects surface of `libghostty-vt`. This is progress toward the
architecture, not Ghostty parity.
...
| Core reuse | Ghostty has a C-ABI core used by its platform GUIs, although its standalone public API remains unstable. | Kiwi's application consumes experimental `kiwi.vt`/`kiwi.vt.headless` Lua API v1. A separate Linux x86_64 C API v1 supplies opaque terminal handles, byte input, resize, logical text, response/effect polling, copied input-mode state, cell/grid render updates, and terminal-mode-aware text/key/mouse/focus/paste encoding; it has no C callback API. | **Partial architectural gap.** Detailed in [KIWI-TO-LIBGHOSTTY](KIWI-TO-LIBGHOSTTY). |

## Terminal-emulation and protocol gaps

| Ghostty capability | Kiwi evidence and current limit | What Kiwi would need |
| --- | --- | --- |
| **Broad xterm-oriented behavior.** Ghostty explicitly targets xterm behavior where appropriate and also follows protocol-origin behavior for modern extensions. Its public reference includes, among many others, full reset (`RIS`), application keypad, DEC alignment test, horizontal margins, palette operations, clipboard OSC, and desktop-notification/progress OSCs. | Kiwi's contract intentionally lists a narrower subset. It now has LNM, application keypad, DECSCA/visible selective erase, xterm reverse-wrap mode 45, declared standard/DEC mode queries (`DECRQM`), bounded XTSAVE/XTRestore, DECRQSS for its implemented status values, and palette operations; reset coverage, most DEC private modes, and xterm/VT100 certification remain incomplete. | Establish a compatibility policy, versioned sequence matrix, test corpus, and per-sequence implementation plan. Do not advertise a feature merely because the parser can retain some state for it. |
...
| **Terminal-side effects as product capabilities.** Ghostty documents title, working-directory, device/size responses, desktop notifications, progress, controlled clipboard write handling, unknown-sequence reporting, and terminal response callbacks. | `kiwi.vt` now offers bounded typed in-process effects and response draining for title/PWD/shell markers/bells/PTY replies, palette/cursor-colour changes, and notification/progress requests; the app applies its own policy. OSC 52 remains default-denied, and notification/progress requests have no native desktop surface. | **Partial gap.** Expand only with explicit permissions, limits, and payload-free diagnostics. |
| **Capability signaling and remote setup.** Ghostty supplies an `xterm-ghostty` terminfo flow and a `+ssh` wrapper that can install it remotely, with a documented fallback when that fails. | Kiwi keeps its 16-colour `TERM=kiwi` contract and now has explicit `kiwi-ssh`, which copies the compiled entry to a remote per-user cache, forwards explicit connection options to both SSH and SCP, then falls back visibly to `xterm-256color`. It has no destination cache, automatic shell wrapper, or real-host validation. | **Partial gap.** Preserve the conservative capability contract and extend the SSH frontend only with a tested argument/policy model. |
| **Keyboard and mouse breadth.** Ghostty documents Kitty keyboard support and its public VT library supplies terminal-configured key, mouse, and focus encoders. | Kiwi has legacy keys plus negotiated Kitty flags 1/2/8/16, bounded GLFW key/text correlation, public host-neutral encoders, X10/normal/button/any mouse tracking with X10, UTF-8, URXVT, SGR-cell, or SGR-pixel encoding, focus reporting, vertical/horizontal wheel reports, and alternate-scroll. Gesture, highlight, locator, and Kitty flag 4 remain excluded. | **Partial gap.** Broaden only through app-facing event normalization and sequence-level conformance tests; preserve local-selection precedence and security rules. |
| **Terminal color/palette operations.** Ghostty's VT reference includes OSC palette and foreground/background/cursor color operations and resets. | Kiwi has bounded OSC 4/10/11/12/104/110/111/112 palette/default/cursor-colour query, update, and reset behavior, with a renderer cursor-colour uniform, while intentionally withholding truecolour terminfo advertisement. Remaining colour controls are absent. | **Partial gap.** Add only tested operations with clear renderer/configuration precedence. |
...
| **Shell integration delivered by the application.** Ghostty can automatically inject integrations for bash, elvish, fish, nushell, and zsh, with prompt navigation/selection and optional SSH wrapping. | Kiwi automatically injects its reversible Bash/Zsh/fish/Nushell scripts for the initial default shell, with configuration disablement and manual switched-shell assets. It has no elvish support, prompt UI, persistence, or automatic `ssh` function wrapping. | **Partial gap.** Broaden only with per-shell startup and remote-session tests. |
...
The Ghostty reference is not exhaustive, and Kiwi's source does contain more than a toy parser: alternate screen, margins, scrollback, LNM/reverse-wrap mode, UTF-8 streaming, grapheme clusters, 256/RGB SGR state, OSC 8, Kitty keyboard negotiation, classic/UTF-8/URXVT/SGR mouse and focus reporting, and limited Kitty graphics all have deterministic tests. The gap is the **supported contract and breadth**, not “Ghostty has a parser while Kiwi does not.”
...
| **Native window behavior and system integration.** Ghostty's macOS application integrates native tabs/splits, Quick Terminal, AppleScript, Quick Look, secure keyboard entry, and state recovery; Linux is a GTK4 application. | Kiwi has GLFW-hosted tabs/splits rather than native widgets, plus a Linux-only bounded AT-SPI provider for the active pane. It has no multi-window/session restoration, validated screen-reader behavior, NSAccessibility, or UI Automation adapter. | **Clear platform/UI gap.** Protocol registration is not a substitute for end-user assistive-technology validation. |
...
| Linux desktop integration and distribution support. | Fedora-oriented source prerequisites, a local Linux x86_64 release artifact with a validated desktop entry, and Nix support. | A supported-install matrix, icon/session behavior, GTK/Wayland/X11 strategy or an equally documented alternative, packaging/release policy, and end-user support boundary. |
...
| Automatic shell injection and SSH convenience workflow. | Initial Bash/Zsh/fish/Nushell injection, explicit remote terminfo upload/fallback, and tested explicit SSH/SCP connection-option forwarding exist. | Automatic wrapping, remote cache policy, additional shells, and real-host tests. |
...
3. **Widen the core boundary from consumer evidence.** `libkiwi-vt` already has a renderer-free Lua API, C cell/grid render updates, C input, and C effect polling; add typed callbacks only with lifecycle tests and a concrete embedding need. See [KIWI-TO-LIBGHOSTTY](KIWI-TO-LIBGHOSTTY).
