...
| --- | --- | --- |
| **Broad xterm-oriented behavior.** Ghostty explicitly targets xterm behavior where appropriate and also follows protocol-origin behavior for modern extensions. Its public reference includes, among many others, full reset (`RIS`), application keypad, DEC alignment test, horizontal margins, palette operations, clipboard OSC, and desktop-notification/progress OSCs. | Kiwi's contract intentionally lists a narrower subset. Its declared limits include incomplete reset coverage, no `DECRQM`, no OSC palette manipulation, no exhaustive DEC private-mode behavior, and no full xterm/VT100 certification. | Establish a compatibility policy, versioned sequence matrix, test corpus, and per-sequence implementation plan. Do not advertise a feature merely because the parser can retain some state for it. |
| **Resize reflow.** The public `libghostty-vt` header describes line wrapping and reflow on resize. | Kiwi explicitly has no runtime width-policy reflow. On resize, retained rows clamp to the new width. | Redesign logical-line/scrollback ownership and define cluster-safe reflow semantics before adding it. This affects selection, search, replay, command regions, images, damage, and renderer caching. |
...
| **Keyboard and mouse breadth.** Ghostty documents Kitty keyboard support and its public VT library supplies terminal-configured key, mouse, and focus encoders. | Kiwi has legacy keys plus a negotiated Kitty disambiguation subset, SGR mouse/focus only, and explicitly excludes legacy, pixel, and gesture mouse protocols and additional Kitty keyboard flags. | Broaden protocol support with app-facing event normalization and sequence-level conformance tests; preserve local-selection precedence and security rules. |
...
The Ghostty reference is not exhaustive, and Kiwi's source does contain more than a toy parser: alternate screen, margins, scrollback, UTF-8 streaming, grapheme clusters, 256/RGB SGR state, OSC 8, Kitty keyboard negotiation, SGR mouse/focus, and limited Kitty graphics all have deterministic tests. The gap is the **supported contract and breadth**, not “Ghostty has a parser while Kiwi does not.”
