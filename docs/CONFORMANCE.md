...
| Native Kitty graphics | `make kitty-graphics-smoke`; or `make conformance-evidence` | Passed structurally on 2026-08-10 with the self-contained direct-PNG client: replay had zero parser errors/ignored/unknown controls, and GPU timestamp output contained both `terminal/kitty_images_under` and `terminal/kitty_images_over`. This verifies the selected protocol stream and pass dispatch, not a broad Kitty-client compatibility claim or pixel-perfect screenshot comparison. |
