...
| modes | IRM; declared RQM/DECRQM queries (IRM; DECCKM, DECOM, DECAWM, DECTCEM, alternate-screen, SGR mouse/focus, bracketed-paste, synchronized-output); DECSCUSR cursor styles; Kitty keyboard level-one disambiguation; SGR mouse/focus reporting | `smkx`/`rmkx`, `civis`/`cnorm`; no cursor-style, bracketed-paste, synchronized-output, extended-keyboard, mouse, or focus terminfo claim |
