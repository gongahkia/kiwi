...
| Cocoa titlebar toolbar | `make cocoa-toolbar-smoke` | Passed on macOS arm64 on 2026-08-23: Kiwi attached a unified AppKit toolbar to its live GLFW/Cocoa window, located the default New Tab item, and dispatched it into the workspace controller. This validates the native control/action bridge, not visual fit-and-finish, interactive toolbar behavior, or native ownership of Kiwi's in-window tab/split workspace. |
