...
A local primary-screen wheel policy is now deterministic on every current
presentation route, including per-session fractional accumulation and inactive
pane focus. Kiwi still has no visible or native scrollbar. That remains a
native-UI gap: the eventual feature must carry a bounded viewport/range model,
pane-local drag ownership, and accessibility exposure, then qualify separately
on Cocoa and GTK instead of being smuggled into terminal state.
