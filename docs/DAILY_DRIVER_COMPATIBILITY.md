...
| Multiple windows and session movement | `Ctrl+Shift+N` opens a fresh default-shell top-level window. On macOS, top-level Kiwi windows join one AppKit tab group while each retains its own WGPU context, compositor, workspace, and PTY set. `Ctrl+Shift+M` transfers the active live PTY/session to a new same-process native window; `Ctrl+Shift+Alt+M` transfers it to the next open window as a workspace tab. `Ctrl+Shift+D` and `Ctrl+Shift+Alt+D` create fresh default-shell counterparts. GLFW events remain process-wide. | `make new-window-smoke` proves same-process window creation; `make session-move-smoke` proves a live PTY handoff across two native windows; `make cocoa-smoke` verifies AppKit tab grouping and independent Metal surfaces; deterministic manager tests cover rollback, duplicate-session creation, and bounds. | partial: interactive native tab behavior, Linux native tabs, user-selectable move targets, and per-target qualification remain. |
...
| Native UI behavior | GLFW owns the custom in-window tab/split workspace. macOS adds a Cocoa global menu, unified titlebar toolbar, searchable AppKit palette, text-configuration Settings action, native top-level tab strip, and an action-only `Kiwi.sdef`/Apple-event bridge; GTK has a shared `GMenu`, text-configuration Settings action, and searchable dialog whose `win.*` entries resolve to the active `GtkApplicationWindow`. Both dispatch the twelve default and configuration-augmented bounded product-action catalogue; the configuration action creates only a missing default comment template and opens it with the host text-file handler. The macOS toolbar exposes New Tab, Split Right, Split Down, Commands, and Settings; its scripting bridge accepts only new window/tab, next tab, close pane, split right/down, reload configuration, and open configuration. Neither route replaces the custom workspace with a native in-window tab/split implementation or adds menu accelerators. | Deterministic configuration tests cover default-path selection and non-overwriting initialization. `make cocoa-smoke` verifies Cocoa menu construction, unified-toolbar dispatch, Settings callback routing, a configured palette entry, AppKit tab grouping, and bundle launch; `make cocoa-menu-smoke`, `make cocoa-toolbar-smoke`, and `make cocoa-palette-smoke` dispatch New Tab through the live controller; `make cocoa-automation-smoke` validates the staged bundle's SDEF and bounded New Tab dispatch. `make gtk-host-check` verifies the GTK ABI; `make gtk-menu-smoke` and `make gtk-palette-smoke` are corresponding graphical-Linux gates. Qualification also requires interactive filtering/menu/window behavior on both targets. | **Partial:** no native in-window workspace tabs/splits, graphical settings editor, full automation object model, terminal-text automation, arbitrary palette execution, external Apple-event permission qualification, Linux native tabs, desktop text-handler qualification, or manual product-chrome qualification. |
