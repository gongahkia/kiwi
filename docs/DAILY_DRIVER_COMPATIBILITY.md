...
| Experimental GTK widget presentation | `KIWI_GTK_PRESENTER=gl` selects one GTK `GtkGLArea` terminal driven by the normal VT/PTy, shaping, input, IME, selection/clipboard, accessibility, and resize policies. It has bounded full-snapshot background, selection/search, alpha-atlas glyph, command-region, and cursor passes. Its atlas cache is generation-aware, but cells and glyphs are still full snapshot uploads. Workspace persistence, tabs, splits, session/window transfer, recording, product actions, and Kitty images are unavailable in this mode. | `make test` verifies producer/consumer damage, retry, cursor scheduling, and controller option gates; `make gtk-gl-renderer-check` compiles the native renderer; `make gtk-gl-wayland-smoke` and `make gtk-gl-x11-smoke` must observe the submitted revision in a real GtkGLArea callback. | **Partial:** no graphical Linux result, visual comparison, performance baseline, colour-management/sRGB result, fractional-scale, device-loss, occlusion/pacing, interactive input, or accessibility qualification. This is not native-tab evidence. |
