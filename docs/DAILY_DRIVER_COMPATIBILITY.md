...
| Multiple windows | `Ctrl+Shift+N` asks the application-owned live window manager for a fresh native window and default-shell session. Each window has an independent GLFW/Cocoa window, WGPU context, compositor, workspace, and PTY set, but all are scheduled in one Kiwi process. | `make new-window-smoke` proves two live native controllers in one process; host-neutral manager tests cover bounds and scheduling. Cross-window session movement, persisted layout restoration, and native menu/window integration remain. | partial |
| Restored layout | No persisted workspace/window state. | Bounded, versioned layout snapshot that excludes terminal text, clipboard, and environment values; migration and corrupted-file tests. | planned |
