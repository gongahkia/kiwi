...
| Multiple windows | `Ctrl+Shift+N` opens an independent native Kiwi window; the platform adapter safely retains multiple GLFW/Cocoa windows and independent WGPU surfaces. Each process owns one workspace. | `make new-window-smoke` covers the source launch path; cross-window lifecycle tests, per-window layout restoration, and native menu/window integration remain. | partial |
