...
| Multiple windows and session movement | `Ctrl+Shift+N` opens a fresh default-shell top-level window. On macOS Kiwi registers it outside the explicit `Ctrl+Shift+T` AppKit group; the verified GLFW/Cocoa default reports `NSWindowTabbingModeDisallowed` for that standalone window. A fresh window or duplicate receives the same bounded local OSC 7 directory policy as a fresh tab; a transferred session keeps its existing PTY. Each has its own WGPU context, compositor, workspace, and PTY set. `Ctrl+Shift+M` transfers the active live PTY/session to a new same-process native window; `Ctrl+Shift+Alt+M` opens a bounded native chooser of up to 15 registered Kiwi windows. `Ctrl+Shift+D` and `Ctrl+Shift+Alt+D` create fresh default-shell counterparts, with the latter using the same chooser. The target is revalidated at transfer time; rejection restores a moved source exactly once or destroys one unadopted duplicate. GLFW events remain process-wide. | Deterministic dispatcher and manager tests cover selection, stale/same target rejection, destination tab-limit rollback, duplicate cleanup, and native-action routing. `make gtk-session-target-smoke` exercises GTK’s native chooser callback. `make new-window-smoke` proves same-process window creation; `make cocoa-smoke` verifies AppKit grouping/selection and independent Metal surfaces; `make session-move-smoke` proves a live PTY handoff across two native windows. | partial: interactive native target choice, native tab behavior, Linux native tabs, and per-target qualification remain. |
| Restored layout | At live-session changes Kiwi writes schema-v2 bounded geometry, tab/split topology, and active tab/pane using a same-directory temporary-file/rename update. It restores that topology with fresh default shells; no terminal contents, PTY state, clipboard, command/environment values, or other host data are serialized. Valid schema-v1 files migrate by dropping their process-local controller IDs; malformed v1 and all future schemas are rejected without restore. | Deterministic current/v1/malformed/future JSON tests plus `make layout-restore-smoke`, which writes a split layout then restores it in a new native session. | partial: manual suspend/restore qualification remains. |
...
| Host notifications and progress | OSC 9 notifications/progress are default-denied. GTK sends configured notifications through window-owned GApplication identifiers and presents configured per-terminal progress through a non-interactive accessible `GtkProgressBar`; states 0/1/2/3/4 mean clear/normal/error/indeterminate/paused. Configured macOS GLFW/Cocoa maps those progress states to a per-window titlebar indicator and submits permitted notifications through `UNUserNotificationCenter`; alert permission is requested only after trusted configuration enables a notification policy. Each accepted notification is replaceable per window and removed at teardown. Separately, `notify-on-command-finish` defaults to `never` and can submit a fixed payload-free completion message after a bounded observed OSC 133 `C`→`D` interval for the same unfocused or any pane/tab. | Deterministic parser/policy/session-isolation tests; `make gtk-progress-smoke` for GTK state/lifecycle; GTK desktop notification delivery; Cocoa bridge build, permission, scheduling, teardown, and desktop delivery; platform-policy qualification. | **Partial:** the GTK adapter/state smoke passed on the current graphical Linux session. GTK notification display, all Cocoa code/build/runtime evidence, and desktop suppression/clearing behavior remain separately unqualified. |
