...
| Native UI behavior | GLFW owns the custom tab/split workspace. On macOS it now installs a Cocoa global main menu whose items call the same logical product actions; the Cocoa window itself remains GLFW-owned. The GTK host owns its `GtkApplication`/`GtkWindow`, but has no documented product-action menu yet. | `make cocoa-smoke` verifies menu construction and the C-to-Lua callback; `make cocoa-menu-smoke` dispatches New Tab and proves that it changes the live workspace. Qualification also requires interactive menu/window behavior on macOS plus GTK product-menu and desktop checks on Linux. | **Partial:** no native tab strip, settings surface, automation model, GTK action menu, or manual product-chrome qualification. |
