...
| `kiwi_vt_render_update_cell` | Copy one logical cell's coordinates, anchor/continuation/span, `0xAARRGGBB` foreground/background, and style flags into `kiwi_vt_render_cell`; obtain UTF-8 display text with the same two-call buffer rule. |
