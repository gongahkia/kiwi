...
Printable ASCII takes a narrow-state fast path: it retains the same singleton cluster semantics while bypassing Unicode-property width classification when the prior cluster cannot be a UAX #29 `Prepend` sequence. The production parser groups only contiguous printable ASCII direct-sink input, then state updates those existing singleton clusters without transient cell tables. Controls, non-ASCII bytes, callback-mode parsing, and `Prepend × ASCII` remain on their established paths. This lowers ordinary ASCII terminal-state cost without making ASCII a separate storage model.
