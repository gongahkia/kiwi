...
Printable ASCII takes a narrow-state fast path: it retains the same singleton cluster semantics while bypassing Unicode-property width classification when the prior cluster cannot be a UAX #29 `Prepend` sequence. `Prepend × ASCII` remains on the general grapheme path. This lowers ordinary ASCII terminal-state cost without making ASCII a separate storage model.
