...
`Pty:read_available` accepts an optional byte budget. The live app uses 4 KiB per service turn by default, configurable through `KIWI_PTY_READ_BUDGET`. Once the terminal parser has opened a Kitty graphics transfer, that pane may additionally read from the bounded and fairly divided `KIWI_KITTY_TRANSFER_READ_BUDGET` (256 KiB by default) in the same turn and caps its event wait at 1 ms until the transfer closes. This exception is limited to an already-recognized image transfer, so ordinary bulk output retains the 4 KiB fairness bound; incomplete transfer chunks also do not request redundant presentation. The reader preserves ordering and returns the remaining bytes on later polls. The real-PTY burst harness enforces the ordinary budget and checks a 250 ms maximum service-turn and 64 MiB retained-heap limit by default.
