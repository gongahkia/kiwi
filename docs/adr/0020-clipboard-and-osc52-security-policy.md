...
- Terminal-originated OSC 52 is denied by default. `osc52-write = true` is the only current opt-in and permits an atomic validated write through the foreground GLFW clipboard bridge. OSC 52 cannot read, clear, trigger a local paste, or produce a response. This applies equally to local programs, SSH sessions, multiplexers, logs, pagers, and replayed terminal output; Kiwi does not infer trust from the source.
- No configuration enables OSC 52 reads or query replies, grants persistent per-program trust, or enables primary/secondary selections or cut buffers.

The OSC 52 write subset is deliberately narrow: `OSC 52 ; c|p|s ; Pd` terminated by BEL or ST, strict RFC 4648 base64 `Pd`, decoded UTF-8 with no NUL, and bounded terminal and platform payload limits. Queries (`?`), malformed delimiter/base64/padding, invalid UTF-8, NUL, over-limit requests, unavailable clipboard, and clipboard errors have no clipboard side effect and no terminal response. A permitted request is all-or-nothing; decoded data is not retained after the platform call.

The current parser has a stricter 4,096-byte control-string cap, which remains the effective OSC 52 limit. Increasing the useful OSC 52 limit requires bounded streaming decoding rather than increasing the generic control-string buffer.
