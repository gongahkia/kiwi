...
| **Scrollback and reflow** | Supports scrollback, resize reflow, and caller-driven bounded incremental compression. | Bounded scrollback exists; resize clamps rather than reflows; no host-driven compression lifecycle. | First choose compatibility semantics. Add logical-line reflow and resource accounting before offering an API that promises it. Expose quotas and eviction/compression events. |
...
7. **The terminal contract remains deliberately narrower than Ghostty's.** Extraction will make Kiwi reusable; it will not give it resize reflow, comprehensive legacy behavior, broad mouse/key protocol support, or full xterm compatibility. Those are independent feature projects.
