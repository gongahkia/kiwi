...
typedef struct kiwi_vt_options {
   ...
   * supply key variants may opt in to bit 4 and supplies each available
   * meaning individually; values must be from 0 through 31. */
  uint32_t keyboard_supported_flags;
  /* Set to one only when the host has an explicit policy permitting OSC 52
   * clipboard queries. Zero denies them. */
  uint32_t osc52_read;
} kiwi_vt_options;
...
typedef struct kiwi_vt_key_event {
   ...
   * physical-position key. Set unavailable values to zero. A flag-4 host
   * supplies each meaning it can derive without inventing another. */
  uint32_t layout_key;
  uint32_t shifted_key;
  uint32_t base_key;
  /* Optional non-control Unicode scalar generated by this event. It is kept
   * separate from `key` because the symbolic special-key range overlaps valid
   * Unicode scalar values. Set zero for a non-text key. */
...
typedef struct kiwi_vt_input_modes {
  ...
  uint32_t backarrow;
  /* XTSHIFTESCAPE request: 0 = no application request, 1 = application
   * allows local Shift override, 2 = application requests Shift capture. */
  uint32_t mouse_shift_escape;
} kiwi_vt_input_modes;
...
enum {
  ...
  KIWI_VT_EFFECT_CLIPBOARD_READ_DENIED = 13,
  KIWI_VT_EFFECT_CLIPBOARD_READ_REQUESTED = 14,
  /* CSS cursor name requested by OSC 22. The host chooses any platform
   * approximation and owns the actual cursor update. */
  KIWI_VT_EFFECT_POINTER_SHAPE_CHANGED = 15,
 ...
 * is otherwise ignored. `unicode_key` is the distinct printable scalar for
 * this event; `key` remains the symbolic/physical key token. The three
 * optional key-variant fields encode Kitty flag 4 only for terminals whose
 * construction mask opted into that flag.
