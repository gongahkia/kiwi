...
- [x] Add truncation and corruption tests at every frame boundary.
...
- [x] Implement placeholder rendering for missing glyphs.
- [x] Add deterministic font-metric fixtures for tests where possible.
...
- [x] Add configurable maximum backend events processed per update.
- [x] Document catch-up behaviour when replay or PTY output exceeds the frame budget.
...
- [x] Implement built-in demonstration commands without imitating a full POSIX environment.
- [x] Implement asynchronous application jobs through explicit scheduled callbacks.
- [x] Ensure sandbox mode has no path to host process execution by default.
