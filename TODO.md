...
- [ ] Add `make check`, `make test`, `make lint`, and `make run` or equivalent commands.
- [ ] Add CI for core tests and linting on macOS and Linux where practical.
- [ ] Add a deterministic test seed mechanism.
- [ ] Record the exact supported LÖVE and LuaJIT versions in the repository.

### Core interfaces

- [ ] Define `Terminal`, `Backend`, `Renderer`, `RecordingReader`, and `Effect` interface contracts as Lua documentation and small stubs.
- [ ] Define typed-enough event tables with constructors and validation.
- [ ] Define a central error taxonomy.
- [ ] Ensure terminal and recording modules do not access global `love.*` APIs.
- [ ] Add import-smoke tests for every core module.

### Exit criteria

- [ ] A clean checkout can run tests using documented commands.
- [ ] Core modules load under plain LuaJIT.
- [ ] The LÖVE app opens a blank terminal viewport without runtime errors.
...
- [ ] Add fixture tests for every supported control sequence.
- [ ] Add property tests showing chunk-boundary invariance.
- [ ] Add property tests showing parser recovery after malformed input.
